...
```{r}
library(ggplot2)
# get the continents of each index
my_gapminder$America <- my_gapminder$continent == "Americas"
my_gapminder$Asia <- my_gapminder$continent == "Asia"
my_gapminder$Europe <- my_gapminder$continent == "Europe"
my_gapminder$Oceania <- my_gapminder$continent == "Oceania"
# predict data
fitted <- x[1] + x[2] * my_gapminder$gdpPercap + 
  as.matrix(my_gapminder[7:10]) %*% x[3:6]
...
ggplot(data = as.data.frame(dat), mapping = aes(x = actual, y = fitted)) +
  geom_jitter() + 
  labs(title = "Actual vs Fitted Values of Model", 
       x = "Actual", y = "Fitted") +
